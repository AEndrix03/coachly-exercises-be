...
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Quadruped Hip Extension') THEN
        RAISE NOTICE 'Already exists: Quadruped Hip Extension'; RETURN;
    END IF;
    v_ex_id := gen_random_uuid();

    INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
    VALUES (v_ex_id,'Quadruped Hip Extension','beginner','compound','static',false,true,'low',false,NULL,'public','active',
