...
BEGIN
    ...
    VALUES (
        v_ex_id, 'Body Saw Plank', 'intermediate', 'compound', 'dynamic',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Tempo Body Saw Plank') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Tempo Body Saw Plank','intermediate','compound','dynamic',false,false,'medium',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Alternating Body Saw Plank') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Alternating Body Saw Plank','advanced','compound','dynamic',false,false,'medium',false,NULL,'public','active',
