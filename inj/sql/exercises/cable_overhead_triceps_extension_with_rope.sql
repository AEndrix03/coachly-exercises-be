-- =============================================================
-- Exercise: Cable Overhead Triceps Extension with Rope
-- =============================================================
DO $$
DECLARE v_ex_id UUID; v_var_id UUID; v_id UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Cable Overhead Triceps Extension with Rope') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = 'Cable Overhead Triceps Extension with Rope';
        ...
        RAISE NOTICE 'Already exists, category updated: Cable Overhead Triceps Extension with Rope'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'Cable Overhead Triceps Extension with Rope','beginner','isolation','push',
        false,false,'low',false,NULL,'public','active',
        jsonb_build_object(
            'it',jsonb_build_object('name','Estensione Tricipiti Sopra Testa con Corda ai Cavi','description','Isolamento tricipiti: tieni il braccio fermo e muovi solo il gomito, portando il carico vicino alla testa e poi estendendo. Mantieni scapole stabili e controlla l''eccentrica per non irritare i gomiti.'),
            'en',jsonb_build_object('name','Cable Overhead Triceps Extension with Rope','description','Triceps isolation: keep the upper arm still and move only at the elbow, lowering the load near the head and then extending. Keep shoulder blades set and control the eccentric to avoid elbow irritation.')
        ), NOW(), NOW());
