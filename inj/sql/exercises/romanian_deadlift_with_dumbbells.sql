...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Single-Leg Romanian Deadlift') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Single-Leg Romanian Deadlift','intermediate','compound','pull',false,false,'low',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Band Romanian Deadlift') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Band Romanian Deadlift','beginner','compound','pull',false,false,'low',false,NULL,'public','active',
