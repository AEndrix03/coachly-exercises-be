-- =============================================================
-- Exercise: Chin-Up (Doorframe Bar)
-- Disciplines: home_workout
-- =============================================================
DO $$
DECLARE v_ex_id UUID; v_var_id UUID; v_id UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Chin-Up (Doorframe Bar)') THEN
        RAISE NOTICE 'Already exists: Chin-Up (Doorframe Bar)'; RETURN;
    END IF;
    v_ex_id := gen_random_uuid();

    INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
    VALUES (v_ex_id,'Chin-Up (Doorframe Bar)','intermediate','compound','pull',false,true,'low',false,NULL,'public','active',
        jsonb_build_object('it',jsonb_build_object('name','Trazioni Supine alla Sbarra','description','Parti da un dead hang con costole giu e scapole attive, poi porta i gomiti verso i fianchi per salire in controllo. Evita lo slancio e chiudi con il mento sopra la sbarra, quindi scendi in modo fluido.'),
                           'en',jsonb_build_object('name','Chin-Up (Doorframe Bar)','description','Start from a dead hang with ribs down and scapulae set, then drive the elbows toward the ribs to pull up under control. Avoid swinging and finish with the chin clearly above the bar before descending smoothly.')),NOW(),NOW());
    ...
    SELECT id INTO v_id FROM exercises.muscle WHERE code='brachialis'; IF v_id IS NOT NULL THEN INSERT INTO exercises.exercise_muscle VALUES(v_ex_id,v_id,'secondary',45,NOW()) ON CONFLICT DO NOTHING; END IF;
    ...
    -- TAGS
    FOR v_id IN SELECT id FROM exercises.tag WHERE code IN('compound','vertical_pull','bilateral','upper_body','no_equipment','home_friendly') LOOP INSERT INTO exercises.exercise_tag VALUES(v_ex_id,v_id,NOW()) ON CONFLICT DO NOTHING; END LOOP;

    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Negative Chin-Up') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Negative Chin-Up','intermediate','compound','static',false,true,'medium',false,NULL,'public','active',
            jsonb_build_object('it',jsonb_build_object('name','Trazione Supina Negativa','description','Variante di Chin-Up (Doorframe Bar) che modifica leva o tempo per lavorare lo stesso schema. Mantieni un setup solido e muoviti in controllo senza perdere posizione.'),
                               'en',jsonb_build_object('name','Negative Chin-Up','description','Variation of Chin-Up (Doorframe Bar) that changes leverage or tempo to target the same pattern. Keep the same tight setup and move under control without losing position.')),NOW(),NOW());
    ...
    ELSE
        SELECT id INTO v_var_id FROM exercises.exercise WHERE name='Negative Chin-Up';
        INSERT INTO exercises.exercise_variation (base_exercise_id, variant_exercise_id, variation_type, difficulty_delta, created_at) VALUES(v_ex_id,v_var_id,'default',-1,NOW()) ON CONFLICT DO NOTHING;
    END IF;
END $$;
