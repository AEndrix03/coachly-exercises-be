-- =============================================================
-- Exercise: Resistance Band Curl
-- =============================================================
DO $$
DECLARE v_ex_id UUID; v_var_id UUID; v_id UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Resistance Band Curl') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = 'Resistance Band Curl';
        ...
        RAISE NOTICE 'Already exists, category updated: Resistance Band Curl'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'Resistance Band Curl','beginner','isolation','pull',
        false,false,'low',false,NULL,'public','active',
        jsonb_build_object(
            'it',jsonb_build_object('name','Curl con Elastico','description','Curl per bicipiti: impugna l''attrezzo, gomiti vicini ai fianchi e spalle stabili, fletti il gomito senza oscillare il busto. Sali in controllo, poi scendi lentamente mantenendo tensione continua.'),
            'en',jsonb_build_object('name','Resistance Band Curl','description','Biceps curl: grip the implement with elbows pinned close to your sides and shoulders stable, flex the elbow without swinging. Curl up under control, then lower slowly keeping constant tension.')
        ), NOW(), NOW());
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Single Arm Band Curl') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,
            overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Single Arm Band Curl','beginner','isolation','pull',true,false,'low',false,
            NULL,'public','active',jsonb_build_object('it',jsonb_build_object('name','Curl Monobraccia con Elastico','description','Variante di Curl con Elastico - Curl per bicipiti: impugna l''attrezzo, gomiti vicini ai fianchi e spalle stabili, fletti il gomito senza oscillare il busto. Sali in controllo, poi scendi lentamente mantenendo tensione continua.'),
                                 'en',jsonb_build_object('name','Single Arm Band Curl','description','Variation of Resistance Band Curl - Biceps curl: grip the implement with elbows pinned close to your sides and shoulders stable, flex the elbow without swinging. Curl up under control, then lower slowly keeping constant tension.')),NOW(),NOW());
