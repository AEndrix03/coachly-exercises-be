...
BEGIN
    ...
    VALUES (
        v_ex_id, 'Band External Rotation Press-Out', 'intermediate', 'compound', 'static',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Half-Kneeling Band External Rotation Press-Out') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Half-Kneeling Band External Rotation Press-Out','beginner','compound','static',false,false,'medium',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Alternating Band External Rotation Press-Out') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Alternating Band External Rotation Press-Out','intermediate','compound','static',false,false,'medium',false,NULL,'public','active',
