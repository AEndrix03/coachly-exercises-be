...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Single-Arm Band Pushdown') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Single-Arm Band Pushdown','beginner','isolation','push',true,false,'low',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Overhead Band Tricep Extension') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Overhead Band Tricep Extension','beginner','isolation','push',false,false,'low',false,NULL,'public','active',
