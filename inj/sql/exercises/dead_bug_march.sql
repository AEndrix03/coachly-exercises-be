...
BEGIN
    ...
    VALUES (
        v_ex_id, 'Dead Bug March', 'intermediate', 'compound', 'static',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Banded Dead Bug March') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Banded Dead Bug March','beginner','compound','static',false,false,'medium',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Wall Dead Bug March') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Wall Dead Bug March','beginner','compound','static',false,false,'medium',false,NULL,'public','active',
