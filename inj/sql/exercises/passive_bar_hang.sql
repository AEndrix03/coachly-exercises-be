...
BEGIN
    ...
    VALUES (v_ex_id,'Passive Bar Hang','','','',false,false,'',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Timed Passive Bar Hang') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Timed Passive Bar Hang','beginner','','',false,false,'',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='One-Arm Passive Bar Hang') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'One-Arm Passive Bar Hang','beginner','','',true,false,'',false,NULL,'public','active',
