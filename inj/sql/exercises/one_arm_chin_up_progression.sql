...
BEGIN
    ...
    VALUES (v_ex_id,'One-Arm Chin-Up Progression','','','',false,false,'',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Assisted One-Arm Chin-Up Progression') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Assisted One-Arm Chin-Up Progression','elite','','',true,false,'',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Archer One-Arm Chin-Up Progression') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Archer One-Arm Chin-Up Progression','elite','','',true,false,'',false,NULL,'public','active',
