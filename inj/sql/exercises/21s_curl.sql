...
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = '21s Curl') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = '21s Curl';
        ...
        RAISE NOTICE 'Already exists, category updated: 21s Curl'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'21s Curl','intermediate','isolation','pull',
        false,false,'low',false,NULL,'public','active',
        jsonb_build_object(
            'it',jsonb_build_object('name','Curl 21s','description','Curl per bicipiti: impugna l''attrezzo, gomiti vicini ai fianchi e spalle stabili, fletti il gomito senza oscillare il busto. Sali in controllo, poi scendi lentamente mantenendo tensione continua.'),
            'en',jsonb_build_object('name','21s Curl','description','Biceps curl: grip the implement with elbows pinned close to your sides and shoulders stable, flex the elbow without swinging. Curl up under control, then lower slowly keeping constant tension.')
        ), NOW(), NOW());
