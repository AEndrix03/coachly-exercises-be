...
BEGIN
    ...
    VALUES (
        v_ex_id, 'Turkish Get-Up to Elbow', 'intermediate', 'compound', 'static',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Tempo Turkish Get-Up to Elbow') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Tempo Turkish Get-Up to Elbow','intermediate','compound','static',false,false,'medium',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Alternating Turkish Get-Up to Elbow') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Alternating Turkish Get-Up to Elbow','advanced','compound','static',false,false,'medium',false,NULL,'public','active',
