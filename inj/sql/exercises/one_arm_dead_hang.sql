...
BEGIN
    ...
    VALUES (v_ex_id,'One-Arm Dead Hang','elite','compound','static',true,true,'very_high',true,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Assisted One-Arm Dead Hang') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Assisted One-Arm Dead Hang','elite','compound','static',true,false,'medium',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Alternating One-Arm Dead Hang') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES (v_var_id,'Alternating One-Arm Dead Hang','elite','compound','static',true,false,'medium',false,NULL,'public','active',
