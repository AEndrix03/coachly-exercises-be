-- =============================================================
-- Exercise: Pronation / Supination
-- =============================================================
DO $$
DECLARE v_ex_id UUID; v_var_id UUID; v_id UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Pronation / Supination') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = 'Pronation / Supination';
        ...
        RAISE NOTICE 'Already exists, category updated: Pronation / Supination'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'Pronation / Supination','beginner','isolation','static',
        ...
        jsonb_build_object(
            ...
            'en',jsonb_build_object('name','Pronation / Supination','description','Forearm work: support the forearm on a bench or thigh and move only the wrist (or rotation) under control. Use light loads and moderate volume to respect the tendons.')
        ), NOW(), NOW());

    -- MUSCLES
    SELECT id INTO v_id FROM exercises.muscle WHERE code='pronator_teres'; IF v_id IS NOT NULL THEN INSERT INTO exercises.exercise_muscle VALUES(v_ex_id,v_id,'primary',80,NOW()) ON CONFLICT DO NOTHING; END IF;
    SELECT id INTO v_id FROM exercises.muscle WHERE code='supinator'; IF v_id IS NOT NULL THEN INSERT INTO exercises.exercise_muscle VALUES(v_ex_id,v_id,'primary',75,NOW()) ON CONFLICT DO NOTHING; END IF;
