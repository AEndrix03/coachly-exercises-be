-- =============================================================
-- Exercise: Manual Neck Resistance - Flexion
-- =============================================================
DO $$
DECLARE v_ex_id UUID; v_var_id UUID; v_id UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Manual Neck Resistance - Flexion') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = 'Manual Neck Resistance - Flexion';
        ...
        RAISE NOTICE 'Already exists, category updated: Manual Neck Resistance - Flexion'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'Manual Neck Resistance - Flexion','beginner','isolation','pull',
        false,false,'low',false,NULL,'public','active',
        jsonb_build_object(
            'it',jsonb_build_object('name','Resistenza Manuale Collo - Flessione','description','Allenamento collo: ripetizioni lente e controllate mantenendo mento rientrato e colonna neutra. Aumenta il carico gradualmente e interrompi se compare dolore acuto.'),
            'en',jsonb_build_object('name','Manual Neck Resistance - Flexion','description','Neck training: use slow controlled reps with a tucked chin and neutral spine. Progress load gradually and stop if you feel sharp pain.')
        ), NOW(), NOW());
