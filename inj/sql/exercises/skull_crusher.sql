...
BEGIN
    ...
    VALUES (v_ex_id,'Skull Crusher','intermediate','isolation','push',false,false,'medium',true,NULL,'public','active',
        jsonb_build_object(
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Overhead Skull Crusher') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Overhead Skull Crusher','beginner','isolation','push',false,false,'low',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Incline Skull Crusher') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Incline Skull Crusher','beginner','isolation','push',false,false,'low',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Cable Skull Crusher') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Cable Skull Crusher','beginner','isolation','push',false,false,'low',false,NULL,'public','active',
