...
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Bear Crawl') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = 'Bear Crawl';
        ...
        RAISE NOTICE 'Already exists, category updated: Bear Crawl'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'Bear Crawl','intermediate','compound','dynamic',
        ...
        jsonb_build_object(
            ...
            'en',jsonb_build_object('name','Bear Crawl','description','Exercise: set a stable position and perform controlled reps focusing on ROM and tension. Keep breathing and posture to reduce compensations.')
        ), NOW(), NOW());
    ...
    -- TAGS
    FOR v_id IN SELECT id FROM exercises.tag WHERE code IN('compound','bilateral','core_focus','hypertrophy','strength','no_equipment','beginner_safe')
