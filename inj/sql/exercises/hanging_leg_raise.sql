...
BEGIN
    ...
    VALUES (v_ex_id,'Hanging Leg Raise','intermediate','compound','dynamic',false,true,'low',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Knee Hanging Leg Raise') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Knee Hanging Leg Raise','intermediate','','',false,false,'',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Toes-to-Bar Hanging Leg Raise') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Toes-to-Bar Hanging Leg Raise','intermediate','','',false,false,'',false,NULL,'public','active',
