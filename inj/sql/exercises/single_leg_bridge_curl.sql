...
BEGIN
    ...
    VALUES (
        v_ex_id, 'Single-Leg Bridge Curl', 'intermediate', 'compound', 'dynamic',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Single-Leg Single-Leg Bridge Curl') THEN
        ...
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Single-Leg Single-Leg Bridge Curl','elite','compound','dynamic',true,false,'medium',false,NULL,'public','active',
    ...
    IF NOT EXISTS (SELECT 1 FROM exercises.exercise WHERE name='Feet-Elevated Single-Leg Bridge Curl') THEN
        v_var_id := gen_random_uuid();
        INSERT INTO exercises.exercise (id,name,difficulty,mechanics,force,unilateral,bodyweight,overall_risk,spotter_required,owner_user_id,visibility,status,translations,created_at,updated_at)
        VALUES(v_var_id,'Feet-Elevated Single-Leg Bridge Curl','elite','compound','dynamic',true,false,'medium',false,NULL,'public','active',
