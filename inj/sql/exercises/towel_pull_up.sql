...
BEGIN
    IF EXISTS (SELECT 1 FROM exercises.exercise WHERE name = 'Towel Pull-Up') THEN
        SELECT id INTO v_ex_id FROM exercises.exercise WHERE name = 'Towel Pull-Up';
        ...
        RAISE NOTICE 'Already exists, category updated: Towel Pull-Up'; RETURN;
    END IF;
    ...
    VALUES (v_ex_id,'Towel Pull-Up','intermediate','compound','pull',
        false,true,'medium',false,NULL,'public','active',
        jsonb_build_object(
            'it',jsonb_build_object('name','Trazioni con Asciugamano','description','Esercizio: imposta una posizione stabile e lavora con ripetizioni controllate, curando ROM e tensione. Mantieni respirazione e postura per ridurre compensi.'),
            'en',jsonb_build_object('name','Towel Pull-Up','description','Exercise: set a stable position and perform controlled reps focusing on ROM and tension. Keep breathing and posture to reduce compensations.')
        ), NOW(), NOW());
    ...
    SELECT id INTO v_id FROM exercises.muscle WHERE code='biceps_long_head'; IF v_id IS NOT NULL THEN INSERT INTO exercises.exercise_muscle VALUES(v_ex_id,v_id,'secondary',50,NOW()) ON CONFLICT DO NOTHING; END IF;
